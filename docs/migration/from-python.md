...
- **Code to schema**: annotate a model's `model_config` with `@context` / `x-sssom` and emit OO-LD via pydantic's `model_json_schema()`.
...
`model_json_schema()` emits a JSON Schema carrying that `@context` and `x-sssom` - a valid OO-LD schema, with no dependency beyond pydantic.
