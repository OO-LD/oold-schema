...
- a schema-level `x-oold-sssom` maps the schema to the OWL/RDFS class it corresponds to.
...
The same as an OO-LD schema - a schema-level `x-oold-sssom` records the class correspondence, the `@context` maps the relation to its predicate and marks it a reference, and `x-oold-range` carries the `sh:class` target:
...
- [Extensions](../guide/extensions.md) - `x-oold-sssom`, `x-oold-range`, and reverse properties.
