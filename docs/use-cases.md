...
class Person(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            ...
            "x-oold-sssom": { "schema:Person": {} },  # ontology correspondence (exactMatch)
        }
    )
...
An OO-LD instance can grow large: a multi-column table or a long time series is naturally an array of row objects, and each row repeats its keys. Two strategies keep this efficient without leaving the JSON / JSON-LD data model; they compose, and the choice is one of scale and access.

**Keep row orientation.** An array of typed row objects is the semantically natural grain: each row is an entity with typed properties and an optional `@id`. A column-of-arrays layout discards per-entity identity and does not survive nested or ragged rows, so it is an analytics view rather than the entity model.

**Inline binary transport (CBOR).** [CBOR](https://www.rfc-editor.org/rfc/rfc8949.html) encodes the same data model as JSON in binary, and the JSON-representable subset round-trips losslessly. Plain CBOR compacts scalars but does not deduplicate repeated keys; [CBOR Packed](https://datatracker.ietf.org/doc/draft-ietf-cbor-packed/) does, and it works on row-oriented data: *substitution* factors repeated keys/values into a shared table (also across nested rows), and *record* compression states the key set once and encodes each row as its values (ideal for homogeneous-repetitive rows such as sensor time series), so rows stay discrete entities. [Typed arrays](https://www.rfc-editor.org/rfc/rfc8746.html) give dense numeric columns where applicable. The packing layer is an IETF draft; generic `gzip`/`zstd` over the bytes is a simpler, opaque alternative, and `@context` key-aliasing (`value` to `v`) is the in-model partial mitigation.
