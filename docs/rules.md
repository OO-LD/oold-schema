...
A frame derivation must not constrain a property whose key aliases a JSON-LD keyword.

A derivation MUST NOT emit a constraining subframe - one carrying `@embed` or any other frame keyword - for a property whose key aliases a JSON-LD keyword, conventionally `id` for `@id` and `type` for `@type` (see [identity](spec/#identity)).
