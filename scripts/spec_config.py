...
TERMINOLOGY = [
    ...
    {"term": "remote context", "lt": "remote context|referenced context|external context",
     "def": "A JSON-LD context referenced by URL, as defined in [[JSON-LD11]] §3.1. The name is "
            "under discussion upstream: [w3c/json-ld-syntax#487]"
            "(https://github.com/w3c/json-ld-syntax/issues/487) proposes avoiding *remote*, "
            "because referencing a context by URL implies no network request - it may be "
            "resolved from a cache or a well-known registry, and the same holds for the `$ref` "
            "half of an OO-LD document. No replacement is settled: that issue suggests "
            "*external context*, while *referenced context* has been put to us directly. "
            "This specification keeps *remote context*, the term its normative reference "
            "defines, and will follow [[JSON-LD11]] once it changes. Both candidates resolve "
            "here so either finds this entry."},
