...
TAB_ASSETS = """  <style>
    /* Colours here are stated so that they work in both of ReSpec's colour schemes.
       ReSpec switches on `prefers-color-scheme` and, independently, on its own
       colour-scheme control, so a rule keyed to the media query alone still breaks for a
       reader who flips that control. Translucent tints over whatever background is in
       effect, and `inherit` / `currentColor` for foregrounds, need neither selector.
       Opaque light values here previously left every one of these below 2.5:1 against
       dark-mode text (see OO-LD/oold-schema#166). */
    .ex-tabs { margin: 1em 0; }
    .ex-tablist { display: flex; gap: .25rem; border-bottom: 1px solid rgba(128, 128, 128, .35); margin-bottom: .5em; }
    .ex-tab { border: 0; background: none; padding: .3em .8em; cursor: pointer; font: inherit; color: inherit; opacity: .65; border-bottom: 2px solid transparent; }
    .ex-tab[aria-selected="true"] { opacity: 1; border-bottom-color: currentColor; font-weight: 600; }
    ...
    .rule { cursor: pointer; }
    .rule:hover { background: rgba(128, 160, 200, .16); }
    .rule:focus-visible { outline: 2px solid currentColor; outline-offset: 2px; }
    /* The brand amber, carried at an alpha that keeps the sentence readable either way:
       18:1 against black on white, 6:1 against light text on a dark background. */
    .rule:target { background: rgba(255, 177, 32, .25); scroll-margin-top: 3rem; }
  </style>
  <script>
