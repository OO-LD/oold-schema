...
Composition is how an [=OO-LD schema=] incorporates multiple independent schemas, each contributing its own properties and JSON-LD term mappings, without requiring a shared parent class. The rules below govern how the resulting `@context` is assembled automatically, so that no post-processing step is needed.
