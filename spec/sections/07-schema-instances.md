...
A materialized `@type` is an ordinary JSON-LD `@type`: its IRIs resolve through the `@context` like any term, so a class name declared as a context term (`"QuantityValue": "qudt:QuantityValue"`) is aliased on export, carries [term synonyms](#synonyms), and drives [frame](#framing) construction. `x-oold-instance-rdf-type` and an inline `type` are thus two ways to reach the same `@type`: the former lets an application keep the type in the schema alone (no `@type` duplicated on every instance) and inject it on export, the latter carries it in self-describing data. Profile-based class-alias selection applies to the materialized `@type` regardless of its source - a type contributed by `x-oold-instance-rdf-type` is aliased per profile exactly as an inline `type` term is. Multiplicity and aliasing are orthogonal: `x-oold-instance-rdf-type` *co-types* - it lists every `rdf:type` an instance asserts at once (e.g. `qudt:QuantityValue` and `schema:QuantitativeValue` together) - while `x-oold-context` on a type term supplies the *synonyms* of a single type, one chosen per profile; a co-typed type may itself carry synonyms, so the two compose.
