// The design system, copied from media/tutorials/shared/theme.ts so the
// explainer, the tutorial series and the talk read as one thing. Segments must
// not redefine colours, type sizes or fonts.
//
// It is a copy rather than an import because media/kit is mechanics only by its
// own charter, and hoisting the design system into it would change what the
// explainer and the series render. Keeping this byte-identical to its source,
// apart from this comment, is what makes it a copy rather than a fork.
//
// FPS, WIDTH and HEIGHT are the composition's, and live in src/timeline.ts with
// the rest of the cut. OVERLAP is deliberately absent: the talk hard-cuts
// between segments so a speaker can advance them by hand.
